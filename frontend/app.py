...
with st.sidebar:
    ...
    st.divider()
    st.caption(f"💬 {len(st.session_state.history)} message(s) in session")
...
if not st.session_state.history:
    ...
    for entry in st.session_state.history:
        ...
        st.markdown(f'<div class="avatar-user msg-time">{entry.get("timestamp","")}</div>', unsafe_allow_html=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        if entry.get("status") == "error":
            st.markdown('<div class="avatar-assistant">🗄️ NL2SQL</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="assistant-bubble">❌ {entry["message"]}</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="avatar-assistant">🗄️ NL2SQL</div>', unsafe_allow_html=True)
            st.markdown(
...
with col1:
    user_input = st.text_input(
        ...
        key=f"chat_input_{st.session_state.input_key}"  # key changes = input clears
    )
with col2:
...
# ── Handle send — only SET pending, never call API here ───────────────────
